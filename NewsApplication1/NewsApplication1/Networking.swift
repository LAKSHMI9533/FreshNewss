...
class Networking{
    ...
    func apiCall(catApiUrl : String = "")->Future<Responce,Error>{
        Future{ promice in
            ...
            var task = URLSession(configuration:.default).dataTask(with: self.baseUrl!) { Data, urlresponce, error in
                if Data != nil{
                    do{
    ...
    func apiCallForImage(uurl : String)->Future<UIImage,Error>{
        Future{ promice in
            if let url = URL(string: uurl){
                if let imageCache = self.cacheData.object(forKey: url.absoluteString as AnyObject) as? UIImage{
                    ...
                    if let data = Data{
                        do{
                            if let reData = try UIImage(data: data){
                                self.cacheData.setObject(reData, forKey: url.absoluteString as AnyObject)
                                promice(.success(reData))
                            }
                        } catch {
                            print(error)
